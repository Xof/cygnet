...
- **Bulk INSERT's per-row loop replicates `_extract_insert_fields` inline for speed.** `_render_bulk_insert` derives the column shape once from row 0 via `_extract_insert_fields`, then for rows 1..N replays that *same* field-order classification inline (a precomputed `(attr, column, is_dbkey, is_pk)` plan + `getattr`) — it does **not** call `_extract_insert_fields` per row (that per-row call was the codebase's single largest CPU cost; ADR `PF1`). Breaks: changing the single-row classification in `_extract_insert_fields` without mirroring it in the bulk loop silently diverges bulk INSERT from single-row INSERT. They are kept byte-identical — same omit/raise rules, and an `AppKey`-None must still short-circuit (raise) *before* a cross-row shape mismatch is reported; `tests/test_bulk_insert_shape.py` is the guard.
